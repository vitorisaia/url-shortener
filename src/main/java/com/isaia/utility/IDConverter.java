...
public class IDConverter {
	...
	private IDConverter() {
		...
		this.initializeIndexToCharTable();
	}
	...
	public String createUniqueID(final Long id) {
		List<Integer> base62ID = this.convertBase10ToBase62ID(id);
	...
	public Long getDictionaryKeyFromUniqueID(final String uniqueID) {
		List<Character> base62Number = new ArrayList<>();
		for (int i = 0; i < uniqueID.length(); ++i) {
			base62Number.add(uniqueID.charAt(i));
		}
		Long dictionaryKey = this.convertBase62ToBase10ID(base62Number);
		return dictionaryKey;
	}

	private Long convertBase62ToBase10ID(final List<Character> ids) {
		long id = 0L;
		int exp = ids.size() - 1;
